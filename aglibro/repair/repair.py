...
from aglibro.util.utils import cleanup_logger, load_jsonl, setup_logger
...
def process_loc(loc, args, swe_bench_data, prev_o, write_lock=None):
    ...
    print(f'args.top_n: {args.top_n}')
    print(f'top_n_content: {topn_content}')
    print(f'file_loc_intervals: {file_loc_intervals}')
    ...
    print(f'greedy_traj: {greedy_traj}')
    print(f'sample_trajs: {sample_trajs}')
    print(f'sample_responses: {sample_responses}')

    count = 0
    while count < args.max_samples:
        logger.info(f"trying the {count + 1}-th sample ...")
...
def main():
    ...
    parser.add_argument(
        ...
        choices=["gpt-4o-2024-05-13", "deepseek-coder", "gpt-4o-mini-2024-07-18", "claude-3-5-sonnet-20241022"],
    )
    parser.add_argument(
