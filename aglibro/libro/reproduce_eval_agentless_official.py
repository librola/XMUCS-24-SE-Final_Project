...
from aglibro.util.postprocess_tests import make_test_script, parse_output, get_logs_eval, MAP_REPO_TO_TEST_PATH, extract_new_file_content
from swebench.harness.constants import (
    PASS_TO_PASS,
    PASS_TO_FAIL,
    FAIL_TO_PASS,
...
def instance_tests_eval(tests, test_spec : TestSpec, args, swe_bench_data, prev_o, client):
    ...
    log_dir = Path(args.output_folder) / "instance_logs" / instance_id
    ...
    tests["final_tests"] = [{
        "test": extract_new_file_content(tests["test_patch"]),
    ...
    test_script_content, test_command = make_test_script(bench_data, spec, "testbed", "/testbed", bench_data["base_commit"], "", False, True)
    ...
    try:
        for i, test_dict in enumerate(final_tests):
            ...
            for pattern in other_patterns:
                if pattern in test_output_after:
                    resolved_after = False
                    break
            if expected_output not in test_output_after:
                resolved_after = False
            report_after = {"resolved": resolved_after}
...
def tests_eval(args):
    ...
    dataset = get_dataset_from_preds(args.dataset, args.split, instances, temp_instances, args.run_id)
