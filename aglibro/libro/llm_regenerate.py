...
def make_messages_from_file(problem_statement, repo, version, test, output, test_command,
    ...
    with open(template_file) as f:
        ...
        current_query = current_query.replace('{{bug_report_content}}', bug_report_content) \
                                     ...
                                     .replace('{{additional_hints}}', MAP_REPO_TO_ADDITIONAL_HINTS.get(repo, "")) \
...
def query_llm_for_gentest(instance, prev_test, prev_output, prev_resolved, model, temperature, template, logger):
    ...
    if prev_resolved:
        template_file = TEMPLATE_DIR+'regenerate_prompt_passed.json'
    else:
        template_file = TEMPLATE_DIR+'regenerate_prompt_failed.json'
...
def regenerate_tests(
    ...
    prev_test = traj[-2]["test"]
    
    trajs = query_llm_for_gentest(instance, prev_test, prev_output, prev_resolved, model, temperature, template, logger)
    res = trajs[0]
    
    messages.extend(res["prompt"])
    messages.append({
        "role": "assistant",
        "content": res["response"]
    })
    traj.extend(copy.deepcopy(messages[-3:]))
