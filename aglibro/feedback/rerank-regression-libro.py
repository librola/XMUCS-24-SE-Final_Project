...
def parse_and_run_instance(instance, test_spec, tests, docker_client, args, existing_instance_ids):
    ...
    container = build_container(test_spec, docker_client, "temp", logger, False, False)
    container.start()
    ...
    try:
        ...
        for i, index in enumerate(weighted_indices):
            ...
            result = run_test(i, 0, instance, container, test_spec, cur_patch, tests, logger, run_regression=True, run_libro=True)
            ...
            #         "belong": patch["belong"]
            #     })
            #     if len(success_patches) >= 10:
            #         break
            
            pass_num = 0
            if result['success'] == 'PASSED':
                pass_num = len(tests)
            elif result['success'] == 'LIBRO':
                pass_num = len(tests) - len(result['res'])
            else:
                pass_num = -1
            iter_result[pass_num].append({
                "id": i,
                "patch": cur_patch,
                "final_libro_test_pass_num": pass_num
            })
            
            result_file = instance_dir / f"result_{i}.json"
    ...
    all_result_file = instance_dir / "all_results.json"
    with open(all_result_file, "w") as f:
        f.write(json.dumps(iter_result, indent=4, cls=SetEncoder))
    
    top10 = []
    for i in range(len(tests), -1, -1):
        iter_result[i].sort(key=lambda x: x['id'])
        top10.extend(iter_result[i])
    top10 = top10[:10]
    ...
    for i in range(10):
        ...
        if i > 0:
            output_file = output_file.replace(".jsonl", f"_{i}.jsonl")
        with open(output_file, "a") as f:
            f.write(json.dumps({
                "model_name_or_path": "aglibro",
                "instance_id": instance_id,
                "patch_type": top10[i]['final_libro_test_pass_num'],
        ...
        with ThreadPoolExecutor(max_workers=args.num_threads) as executor:
            ...
            futures = {
                executor.submit(
                    ...
                    [tests for tests in all_tests if tests['instance_id'] == instance_id][0],
                    client,
                    args,
