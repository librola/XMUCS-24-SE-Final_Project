...
def copy_to_container(container: Container, src: Path, dst: Path):
    ...
    container.exec_run(f"mv {dst.parent}/{src.name} {dst}")
...
def exec_run_with_timeout(container, cmd, timeout: int|None=60):
    ...
    def run_command():
        ...
        try:
            ...
            for chunk in exec_stream:
                decoded_chunk = None
                for encoding in ["utf-8", "ISO-8859-1", "MacRoman", "Windows-1252"]:
                    if can_decode(chunk, encoding):
                        decoded_chunk = chunk.decode(encoding)
                        if encoding != "utf-8":
                            with open("errorlog.txt", "a") as f:
                                f.write(f"Decoding chunk in {container.name} with encoding {encoding}.\n")
                        break
                if decoded_chunk is None:
                    try:
                        ...
                        decoded_chunk = chunk.decode(encoding)
                        with open("errorlog.txt", "a") as f:
                            f.write(f"Error decoding chunk in {container.name}, the encoding is {encoding}.\n")
                    except UnicodeDecodeError as e:
                        with open("errorlog.txt", "a") as f:
                            f.write(f"Error decoding chunk in {container.name}, the encoding is unknown.\n")
                        decoded_chunk = chunk.decode("utf-8", errors="replace")
                exec_result += decoded_chunk
        except Exception as e:
            exception = e
    ...
    if thread.is_alive():
        if exec_id is not None:
            ...
            if "pytest-dev__pytest-7432" in container.name:
                container.exec_run(f"pkill -KILL pytest", detach=False)
        timed_out = True
    end_time = time.time()
